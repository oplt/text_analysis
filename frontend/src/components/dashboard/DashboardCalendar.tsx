        ...
        <SectionCard title="Workspace calendar" description="Switch between focused day planning, weekly scheduling, monthly scanning, and a 12-month horizon." action={
            <Stack spacing={1} alignItems={{ xs: "stretch", sm: "flex-end" }}>
                ...
                <Stack direction="row" spacing={0.75} justifyContent="flex-end">
                    <Button size="small" variant="text" startIcon={<ChevronLeftIcon />} onClick={() => m.setAnchorDate((current) => shiftCalendarAnchor(current, m.viewMode, -1))}>Back</Button>
                    <Button size="small" variant="text" endIcon={<ChevronRightIcon />} onClick={() => m.setAnchorDate((current) => shiftCalendarAnchor(current, m.viewMode, 1))}>Forward</Button>
                </Stack>
            </Stack>
        }>
            {m.itemsQuery.isError && <Box sx={{ mb: 2 }}><QueryErrorAlert error={m.itemsQuery.error} fallback="Failed to load calendar items." onRetry={() => void m.itemsQuery.refetch()} /></Box>}
            {m.itemsQuery.isLoading ? (m.viewMode === "day" || m.viewMode === "week" ? <Stack spacing={2}><Skeleton variant="rounded" height={340} sx={{ borderRadius: 4 }} /><Skeleton variant="rounded" height={220} sx={{ borderRadius: 4 }} /></Stack> : <Box sx={{ display: "grid", gap: 1.5, gridTemplateColumns: getMonthGridColumns(m.viewMode) }}>{m.visibleMonths.map((month) => <Skeleton key={month.format("YYYY-MM")} variant="rounded" height={m.viewMode === "twelve_month" ? 360 : 420} sx={{ borderRadius: 4 }} />)}</Box>) : <CalendarViews m={m} />}
        </SectionCard>
