...
export function MetadataFilterBar({ corpusId, value, onChange }: { corpusId: string; value: Record<string, string>; onChange: (value: Record<string, string>) => void }) {
    const facets = useQuery({ queryKey: ["text-research", "metadata-facets", corpusId], queryFn: () => getCorpusMetadataFacets(corpusId), enabled: Boolean(corpusId), staleTime: QUERY_STALE_TIMES.researchReference });
